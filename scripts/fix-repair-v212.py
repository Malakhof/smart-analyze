...
# MAIN
long_uuids = json.load(open('/tmp/backfill-839/v212-changed-uuids.json'))
transcripts = {}
...
# Save SQL
with open('/tmp/backfill-839/repair-fix-v212.sql', 'w') as f:
    f.write("BEGIN;\n")
    for uid, rep in results.items():
        safe = rep.replace("'", "''")
        f.write(f"UPDATE \"CallRecord\" SET \"transcriptRepaired\"='{safe}' WHERE \"pbxUuid\"='{uid}';\n")
    f.write("COMMIT;\n")
print(f"SQL: /tmp/backfill-839/repair-fix-v212.sql")
